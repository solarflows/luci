...
return baseclass.extend({
			...
			for(var idx in this.params.wifi.devices[i]) {
				...
				if ('progress' == idx) {
					container_content = E('div', { 'class' : 'td device-info' }, [
						E('div', { 'class': 'cbi-progressbar', 'title': 'RSSI: ' + Math.round(device[idx].value.qualite) + '% (' + device[idx].value.rssi + 'dBm)'  }, [
							E('div', { 'class': device[idx].value.style, 'style': 'width: '+device[idx].value.qualite+'%'}),
					])
					]);
	...
	renderUpdateData: function(radios, networks, hosthints) {
		...
		for (var i = 0; i < radios.sort(function(a, b) { return a.getName().localeCompare(b.getName()) }).length; i++) {
		...
		for (var i = 0; i < networks.length; i++) {
			for (var k = 0; k < networks[i].assoclist.length; k++) {
				...
				var q = Math.min(100, Math.max(0, 100 * ((bss.signal - (bss.noise ? bss.noise : defaultNF)) / (defaultCeil - (bss.noise ? bss.noise : defaultNF)))));
